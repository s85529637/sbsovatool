...
public partial class Upd_ChongZhi : System.Web.UI.Page
{
    String HomePage = "Upd_ChongZhi.aspx";
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!string.IsNullOrEmpty(Request.QueryString["err"]))
        {
            ...
            {
                Response.Write("<script>alert('請檢查驗證碼設定!!');location.href='Upd_ChongZhi.aspx';</script>");
                Response.End();
            }
            else if (Request.QueryString["err"].ToString() == "2")
            {
                Response.Write("<script>alert('驗證碼有誤!!');location.href='Upd_ChongZhi.aspx';</script>");
                Response.End();
            }
        }
    }
    ...
    protected void btnInquire_Click(object sender, EventArgs e)
    {
        ...
        string sqlCommand = @"IF EXISTS (select 1 from T_Club where Club_Ename =@Club_Ename)
                                  BEGIN
                                      SELECT Club_Id,Club_Ename,ChongZhi,
                                      CASE LOGIN
                                      WHEN 0 THEN '否'
                                  　　ELSE '是' END　as LOGIN 
                                  ...
                                  ELSE   
                                  BEGIN
                                  SELECT 0 as Club_Id,0 as Club_Ename,0 as ChongZhi,0 as LOGIN
                                  END";
    ...
    protected void btnQuery_Click(object sender, EventArgs e)
    {
        ...
        string ChongZhi = TextBox1.Text.Trim();
        if (ChongZhi != "")
        {
            if (CreateAccount == "")
            {
            ...
            if (!string.IsNullOrEmpty(TextBox2.Value))
            {
                if (TextBox2.Value.ToString() != "")
                {
                    ...
                    {
                        Response.Redirect(string.Format("{0}?err=2", "Upd_ChongZhi.aspx"));
                    }
    ...
    public class Card
    {
        ...
        public double ChongZhi { get; set; }
        public string Login { get; set; }
        public double OldChongZhi { get; set; }
    ...
    private void DataSearch()
    {
        ...
        string connString = reader.GetValue("Main.ConnectionString", typeof(string)).ToString();
        string Club_Ename = txtAccount.Text.Trim();
        double ChongZhi = Double.Parse(TextBox1.Text.Trim());
        List<Card> dt = null;
        string sqlCommand = @"EXEC UPD_ChongZhi @Club_Ename,@ChongZhi";
        using (var conn = new SqlConnection(connString))
        {
            dt = conn.Query<Card>(sqlCommand, new { @ChongZhi = ChongZhi, @Club_Ename = Club_Ename }).ToList();
            dv.DataSource = dt;
            dv.DataBind();
        }
        ...
        {
            string mailBody = "會員帳號:" + dt[0].Club_Ename + "<br>修改後額度:" + dt[0].ChongZhi + "<br>修改前額度:" + dt[0].OldChongZhi + "<br>修改時間:" + dt[0].UpdDate;
            SendMailByGmail(mailBody);
            ClientScript.RegisterStartupScript(GetType(), "hwa", "alert('修改成功');", true);
            logger.Information("修改額度資訊:{System},{Application},{DB環境},{會員帳號},{修改前額度},{修改後額度}", "Sovatool", "修改充值額度","舊公測",dt[0].Club_Ename, dt[0].ChongZhi, dt[0].OldChongZhi);
        }
    }
}